...
REQUIRED_PYTHON_MODULES = ("textual", "yt_dlp", "mutagen")


def missing_runtime_dependencies(
    modules: tuple[str, ...] = REQUIRED_PYTHON_MODULES,
) -> list[str]:
    missing: list[str] = []
    for name in modules:
        try:
...
def format_dependency_error(missing: list[str]) -> str:
    names = ", ".join(missing)
    return (
        f"Missing required component(s): {names}\n"
        "Install everything with: ./install.sh"
    )
...
def require_runtime_dependencies() -> None:
    ...
    if missing:
        print(format_dependency_error(missing), file=sys.stderr)
        raise SystemExit(1)
...
def yt_dlp_cmd(*args: str) -> list[str]:
    """Run yt-dlp as a module of this interpreter so extras like mutagen match."""
    return [sys.executable, "-m", "yt_dlp", *args]


def fetch_playlist(url: str) -> list[Track]:
    proc = subprocess.run(
        yt_dlp_cmd("--flat-playlist", "--dump-json", "--no-warnings", url),
...
def download_track(track: Track, music_dir: str, fmt: str) -> DownloadResult:
    ...
    proc = subprocess.run(
        yt_dlp_cmd(
            "-x", "-f", "bestaudio/best", "--audio-format", fmt,
